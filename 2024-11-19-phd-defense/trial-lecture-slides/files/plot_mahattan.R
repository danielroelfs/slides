...
manhplot <- gwas_data |> 
    mutate(
        sig = ifelse(p < 5e-8, "sig", "notsig"),
        chr_color = ifelse(chr %% 2 > 0, "#e67467", "#8a382d")
        ) |> 
    ggplot(aes(x = bp_cum, y = -log10(p), color = chr_color, size = -log10(p))) +
...
ggsave("files/manhattan_plot.png", plot = manhplot, width = 6, height = 3, dpi = 300, bg = "transparent")
