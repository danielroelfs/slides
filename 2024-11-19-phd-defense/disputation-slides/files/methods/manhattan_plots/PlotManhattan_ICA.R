...
for (i in seq(5)) {
  ...
  manhplot <- gwas_data %>% 
    ...
    geom_richtext(data = tibble(), aes(x = 1e7, y = ylim - 2, label = str_glue("**IC{i}**")), 
                  color = ic_colors[i], size = 18, fill = "transparent", family = "Poppins", hjust = 0, vjust = 1,
    ...
    theme_void() + 
    theme(
      panel.border = element_rect(color = ic_colors[i], fill = "transparent", linewidth = 6),
      panel.background = element_rect(fill = "transparent"),
      legend.position = "none"
    )
  
  ggsave(str_glue("manhattan_plot_ic{i}.png"), plot = manhplot, width = 6, height = 3, dpi = 300, bg = "#EFEFEF")
