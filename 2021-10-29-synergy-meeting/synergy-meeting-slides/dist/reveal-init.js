...
(function () {
	...
	// LOGO ANIMATION
	//
	//TweenMax.staggerFrom("[data-bit]", 1.5, { y: 25, autoAlpha: 0 }, -0.15);
	//TweenMax.from("[data-allBits]", 1.5, { y: -100, ease: Back.easeOut.config(4) });
	//Reveal.addEventListener( 'ready', function() {
	//	TweenMax.staggerFrom("[data-bit]", 1, { y: 25, autoAlpha: 0 }, -0.15);
	//	TweenMax.from("[data-allBits]", 1.5, { y: -100, ease: Back.easeOut.config(4) });
	//} );
	Reveal.addEventListener( 'slidechanged', function() {
		TweenMax.staggerFrom("[data-bit]", 1.5, { y: 50, autoAlpha: 0 }, -0.15);
		TweenMax.from("[data-allBits]", 1.5, { y: -100, ease: Back.easeOut.config(4) });
	} );
